...
import { getAuth, signInWithEmailAndPassword, onAuthStateChanged, signInAnonymously, signOut } from 'firebase/auth'; // Added signInWithEmailAndPassword
...
let currentUserId = null; // Renamed to avoid conflict with `user.uid` directly
...
const FirebaseProvider = ({ children }) => {
  ...
  // For the initial setup, you can temporarily set it to a known UID if you're testing.
  // After initial anonymous login, check the User/Admin Login tab for your UID.
  ...
  useEffect(() => {
    ...
    initializeFirebase();
  }, [initialAuthToken, firebaseConfig]); // Add dependencies to useEffect
...
const PoetrySubmissionForm = ({ labels }) => {
  ...
  const [language, setLanguage] = useState('English'); // New: Language selection
      ...
      onConfirm: async () => {
        try {
          await updateDoc(doc(db, `artifacts/${appId}/public/data/poetries`, poetryId), {
            status: 'approved',
            author: 'UFAQ Official' // Set the official author name
          });
          setShowModal(false);
          setModalContent({
            title: "Success",
    ...
    <div className="p-8 bg-white dark:bg-gray-800 rounded-xl shadow-lg my-8">
      ...
      <div className="mb-8">
        ...
        <ul className="space-y-2">
          {labels.map((label) => (
            <li key={label.id} className="flex flex-col sm:flex-row items-center justify-between p-3 bg-gray-100 dark:bg-gray-700 rounded-md">
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const checkAndAddDefaultLabels = async () => {
      const labelsColRef = collection(db, `artifacts/${appId}/public/data/labels`);
  ...
  // Pre-populate initial poetries if the collection is empty
  useEffect(() => {
    if (!firebaseReady || !db) return;

    const checkAndAddDefaultPoetries = async () => {
      const poetriesColRef = collection(db, `artifacts/${appId}/public/data/poetries`);
...
It needs admin approval to be visible on the main page.`,
  ...
  const filteredPoetries = poetries
    ...
    .filter(p =>
      ...
      (p.language && p.language.toLowerCase().includes(searchTerm.toLowerCase())) || // Search by language
      (p.label && p.label.toLowerCase().includes(searchTerm.toLowerCase())) // Ensure label exists
    );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans p-4">
      ...
      <main className="container mx-auto max-w-4xl p-4">
        {showAdminSetupInfo && currentPage === 'home' && user && (
          <div className="text-center text-sm text-gray-500 dark:text-gray-400 mb-8 p-4 bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 rounded-lg flex flex-col sm:flex-row items-center justify-center">
      ...
      <footer className="text-center text-gray-600 dark:text-gray-400 mt-12 text-sm opacity-80">
